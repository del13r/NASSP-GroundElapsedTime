...
void GroundElapsedTimeExportStep(double simt)
{
	...
	// Unsupported vessels have no mission clock. Use zero for that case,
	// but preserve a real vessel's negative countdown.
	if (!haveGET)
		mt = 0.0;
	...
	FormatGroundElapsedTimeDays(mt, days, sizeof(days));
